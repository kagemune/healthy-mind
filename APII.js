    ...
    try {
      ...
      if (jsonMatch) {
        ...
        moderationResult = {
          clasificacion: "rechazado",
          explicacion:
            "No se pudo analizar correctamente el contenido. Por precaución, el comentario no será publicado.",
        };
      }
    } catch (parseError) {
      ...
      moderationResult = {
        clasificacion: "rechazado",
        explicacion:
          "Error en el sistema de moderación. Por precaución, el comentario no será publicado.",
      };
    }
  ...
  try {
    ...
    const debouncedPromise = new Promise((resolve, reject) => {
      const timeout = setTimeout(async () => {
        try {
          console.log(
            `Procesando petición después de ${DEBOUNCE_DELAY}ms de espera`
          );
...
app.post("/preguntar", async (req, res) => {
  ...
  res.json({
    respuesta:
      "Este endpoint ha sido deprecado. Por favor usa /moderar para el sistema de moderación de contenido.",
  });
});
...
app.listen(PORT, "0.0.0.0", () => {
  console.log(
    `Servidor de moderación automática corriendo en el puerto ${PORT}`
  );
  console.log(
    `Debounce configurado a ${DEBOUNCE_DELAY}ms para optimizar llamadas a la API`
  );
  console.log(
    `Cache activado con duración de ${CACHE_DURATION / 1000 / 60} minutos`
  );
